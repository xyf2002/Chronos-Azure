...
for nic_id in $(az network nic list -g "$RESOURCE_GROUP" --query "[?ipConfigurations[0].privateIpAddress!=null].id" -o tsv | tr -d '\r'); do
...
for bastion_id in $(az network bastion list -g "$RESOURCE_GROUP" --query "[].id" -o tsv | tr -d '\r'); do
...
for vm in $(az vm list -g "$RESOURCE_GROUP" --query "[].name" -o tsv | tr -d '\r'); do
    nic_id=$(az vm show -g "$RESOURCE_GROUP" -n "$vm" --query "networkProfile.networkInterfaces[0].id" -o tsv | tr -d '\r')
    vnet_id=$(az network nic show --ids "$nic_id" --query "ipConfigurations[0].subnet.id" -o tsv | tr -d '\r' | awk -F'/subnets/' '{print $1}')
